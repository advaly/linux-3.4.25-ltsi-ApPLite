/*
 * drivers/staging/applite/veu/applite_veu_reset_power_seq.c
 *
...
void veu_internal_vip4_reset(struct veu_driver *pdev, int on)
{
	if (on) {
		/* reset on */
		...
		reg_write_reset_power(
			"SoftResetOn_vcodecSS_0",
			VAL_VEU_RSTON_SOFTRESETON_VCODECSS_0,
			REG_PMU(pdev, SoftResetOn_vcodecSS_0_OFFS));

		/* ClockGatingOn_vcodecSS_0 (clock stop) */
#if 1
		clk_disable_unprepare(pdev->clk);
		DBG(pdev, "ClockGatingOn:0x%08X\n",
			__raw_readl(REG_PMU(pdev, ClockGatingOn_vcodecSS_0_OFFS)));
#else
		reg_write_reset_power(
			"ClockGatingOn_vcodecSS_0",
			VAL_VEU_RSTON_CLOCKGATINGON_VCODECSS_0,
		...
		DBG(pdev, "ClockGatingOff:0x%08X\n",
			__raw_readl(REG_PMU(pdev, ClockGatingOff_vcodecSS_0_OFFS)));
#else
		reg_write_reset_power(
			"ClockGatingOff_vcodecSS_0",
			VAL_VEU_RSTOF_CLOCKGATINGOFF_VCODECSS_0,
		...
		reg_write_reset_power(
			"SoftResetOff_vcodecSS_0",
			VAL_VEU_RSTOF_SOFTRESETOFF_VCODECSS_0,
			REG_PMU(pdev, SoftResetOff_vcodecSS_0_OFFS));
	}

}

void veu_internal_video_power(struct veu_driver *pdev, int on)
{
	u32 v;
	...
	if (on) {
		...
		/* power off */
		if (pdev->pctrl == VEU_POWER_CTRL_ON)
			return;
