...
{
    return readw((const volatile void __iomem *)ulAddr);
}
...
{
    return readb((const volatile void __iomem *)ulAddr);
}
...
{
    writel(ulValue, (volatile void __iomem *)ulAddr);
    return;
}
...
{
    writew(usValue, (volatile void __iomem *)ulAddr);
    return;
}
...
{
    writeb(ucValue, (volatile void __iomem *)ulAddr);
    return;
}
...
os_sts_e os_deleteLock(os_lock_t **pLock)
{
    ...
    for(i = 0; i < _MAX_NUM_RES; i++)
    {
        ...
        if(*pLock == &os_lk[i] && (os_lk[i].uiLockAvailable == _IN_USE))
        {
            os_lk[i].uiLockAvailable = _NOT_IN_USE;
            uiLockFound = 1;
            break;
...
os_sts_e os_deleteTimer(os_timer_t **pTimer)
{
    ...
    for(i = 0; i < _MAX_NUM_RES; i++)
    {
        ...
        if(*pTimer == &os_tmr[i] && (os_tmr[i].uiTimerAvailable  == _IN_USE))
        {
            os_tmr[i].uiTimerAvailable  = _NOT_IN_USE;
            uiTimerFound = 1;
            break;
