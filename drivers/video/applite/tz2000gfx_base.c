/*
 * linux/drivers/video/applite/tz2000gfx_base.c
 *
...
extern __u32 tz2000fb_sram_vtop_reg_base;
#endif
...
#if IS_ENABLED(CONFIG_FB_APPLITE_BASIC_RESOURCE_MINIMAL_SRAM)
	if (CPU_SRAM_REG_BASE > tz2000fb_sram_vtop_reg_base)
		cmd = C_LGPR(DE_DST_ADR_REG, virt_to_phys((void *)(dl->dst_addr)) + (CPU_SRAM_REG_BASE - tz2000fb_sram_vtop_reg_base));
	else
		cmd = C_LGPR(DE_DST_ADR_REG, virt_to_phys((void *)(dl->dst_addr)) - (tz2000fb_sram_vtop_reg_base - CPU_SRAM_REG_BASE));
#else
...
#if IS_ENABLED(CONFIG_FB_APPLITE_BASIC_RESOURCE_MINIMAL_SRAM)
	if (CPU_SRAM_REG_BASE > tz2000fb_sram_vtop_reg_base)
		cmd = C_LGPR(DE_DST_ADR_REG, virt_to_phys((void *)(dt->dst_addr)) + (CPU_SRAM_REG_BASE - tz2000fb_sram_vtop_reg_base));
	else
		cmd = C_LGPR(DE_DST_ADR_REG, virt_to_phys((void *)(dt->dst_addr)) - (tz2000fb_sram_vtop_reg_base - CPU_SRAM_REG_BASE));
#else
...
#if IS_ENABLED(CONFIG_FB_APPLITE_BASIC_RESOURCE_MINIMAL_SRAM)
			if (CPU_SRAM_REG_BASE > tz2000fb_sram_vtop_reg_base)
				cmd = C_LGPR(BE_CLIP_START_REG, virt_to_phys((void *)(blt->clip_addr)) + (CPU_SRAM_REG_BASE - tz2000fb_sram_vtop_reg_base));
			else
				cmd = C_LGPR(BE_CLIP_START_REG, virt_to_phys((void *)(blt->clip_addr)) - (tz2000fb_sram_vtop_reg_base - CPU_SRAM_REG_BASE));
#else
...
#if IS_ENABLED(CONFIG_FB_APPLITE_BASIC_RESOURCE_MINIMAL_SRAM)
	if (CPU_SRAM_REG_BASE > tz2000fb_sram_vtop_reg_base)
		cmd = C_LGPR(BE_SRC_ADR_REG, virt_to_phys((void *)(prm->src_addr)) + (CPU_SRAM_REG_BASE - tz2000fb_sram_vtop_reg_base));
	else
		cmd = C_LGPR(BE_SRC_ADR_REG, virt_to_phys((void *)(prm->src_addr)) - (tz2000fb_sram_vtop_reg_base - CPU_SRAM_REG_BASE));
#else
...
#if IS_ENABLED(CONFIG_FB_APPLITE_BASIC_RESOURCE_MINIMAL_SRAM)
	if (CPU_SRAM_REG_BASE > tz2000fb_sram_vtop_reg_base)
		cmd = C_LGPR(BE_DST_ADR_REG, virt_to_phys((void *)(prm->dst_addr)) + (CPU_SRAM_REG_BASE - tz2000fb_sram_vtop_reg_base));
	else
		cmd = C_LGPR(BE_DST_ADR_REG, virt_to_phys((void *)(prm->dst_addr)) - (tz2000fb_sram_vtop_reg_base - CPU_SRAM_REG_BASE));
#else
...
#if IS_ENABLED(CONFIG_FB_APPLITE_BASIC_RESOURCE_MINIMAL_SRAM)
	if (CPU_SRAM_REG_BASE > tz2000fb_sram_vtop_reg_base)
		cmd = C_LGPR(TE_SRC_ADR_REG, virt_to_phys((void *)(trans->src_addr)) + (CPU_SRAM_REG_BASE - tz2000fb_sram_vtop_reg_base));
	else
		cmd = C_LGPR(TE_SRC_ADR_REG, virt_to_phys((void *)(trans->src_addr)) - (tz2000fb_sram_vtop_reg_base - CPU_SRAM_REG_BASE));
#else
...
#if IS_ENABLED(CONFIG_FB_APPLITE_BASIC_RESOURCE_MINIMAL_SRAM)
	if (CPU_SRAM_REG_BASE > tz2000fb_sram_vtop_reg_base)
		cmd = C_LGPR(TE_LUT_ADR_REG, virt_to_phys((void *)(trans->lut_addr)) + (CPU_SRAM_REG_BASE - tz2000fb_sram_vtop_reg_base));
	else
		cmd = C_LGPR(TE_LUT_ADR_REG, virt_to_phys((void *)(trans->lut_addr)) - (tz2000fb_sram_vtop_reg_base - CPU_SRAM_REG_BASE));
#else
...
#if IS_ENABLED(CONFIG_FB_APPLITE_BASIC_RESOURCE_MINIMAL_SRAM)
	if (CPU_SRAM_REG_BASE > tz2000fb_sram_vtop_reg_base)
		cmd = C_LGPR(TE_DST_ADR_REG, virt_to_phys((void *)(trans->dst_addr)) + (CPU_SRAM_REG_BASE - tz2000fb_sram_vtop_reg_base));
	else
		cmd = C_LGPR(TE_DST_ADR_REG, virt_to_phys((void *)(trans->dst_addr)) - (tz2000fb_sram_vtop_reg_base - CPU_SRAM_REG_BASE));
#else
...
#if IS_ENABLED(CONFIG_FB_APPLITE_BASIC_RESOURCE_MINIMAL_SRAM)
	if (CPU_SRAM_REG_BASE > tz2000fb_sram_vtop_reg_base)
		cmd = C_LGPR(RE_SRC_ADR_REG, virt_to_phys((void *)(rot->src_addr)) + (CPU_SRAM_REG_BASE - tz2000fb_sram_vtop_reg_base));
	else
		cmd = C_LGPR(RE_SRC_ADR_REG, virt_to_phys((void *)(rot->src_addr)) - (tz2000fb_sram_vtop_reg_base - CPU_SRAM_REG_BASE));
#else
...
#if IS_ENABLED(CONFIG_FB_APPLITE_BASIC_RESOURCE_MINIMAL_SRAM)
	if (CPU_SRAM_REG_BASE > tz2000fb_sram_vtop_reg_base)
		cmd = C_LGPR(RE_DST_ADR_REG, virt_to_phys((void *)(rot->dst_addr)) + (CPU_SRAM_REG_BASE - tz2000fb_sram_vtop_reg_base));
	else
		cmd = C_LGPR(RE_DST_ADR_REG, virt_to_phys((void *)(rot->dst_addr)) - (tz2000fb_sram_vtop_reg_base - CPU_SRAM_REG_BASE));
#else
...
tz2000gfx_probe(struct platform_device *pdev)
#else
int
tz2000gfx_init(void)
#endif /* !defined(_MERGE_FBDEV_) */
{
	...
	/* map register */
#if defined(_MERGE_FBDEV_)
	tz2000_drv.gfxbase = (__u8 __iomem *)priv.gfx_regs;
	tz2000_drv.pmubase = (__u8 __iomem *)priv.pmu_regs;
#else
...
tz2000gfx_remove(struct platform_device *pdev)
#else
void
tz2000gfx_exit(void)
#endif /* !defined(_MERGE_FBDEV_) */
...
static const struct dev_pm_ops tz2000gfx_pmops = {
	.suspend = tz2000gfx_suspend,
	.resume = tz2000gfx_resume,
	.freeze = tz2000gfx_suspend,
	.thaw = tz2000gfx_resume,
	.restore = tz2000gfx_resume,
#if IS_ENABLED(CONFIG_PM_RUNTIME)
	.runtime_suspend = tz2000gfx_suspend,
	.runtime_resume = tz2000gfx_resume,
#endif /* IS_ENABLED(CONFIG_PM_RUNTIME) */
};

static struct platform_driver tz2000gfx_driver = {
	.probe	= tz2000gfx_probe,
	.remove = tz2000gfx_remove,
	.driver = {
		.name	= "tz2000gfx",
		.pm = &tz2000gfx_pmops,
	},
...
tz2000gfx_init(void)
{
	...
	ret = platform_driver_register(&tz2000gfx_driver);
...
tz2000gfx_exit(void)
{
	...
	platform_driver_unregister(&tz2000gfx_driver);
