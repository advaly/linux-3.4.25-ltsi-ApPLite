/*
 * linux/drivers/input/touchscreen/it7260_mts.c
 *
...
struct it7260_ts_data {
	unsigned short xpos[3];
	unsigned short ypos[3];
...
static void it7260_ts_poscheck(struct work_struct *work)
{
	...
	/* finger 0 */
	if (buf[0] & 0x01) {
		xpos[0] = IT7260_X_RESOLUTION - (((buf[3] & 0x0F) << 8) | buf[2]);
		ypos[0] = IT7260_Y_RESOLUTION - (((buf[3] & 0xF0) << 4) | buf[4]);
		event[0] = buf[5] & 0x0F;
	}

	/* finger 1 */
	if (buf[0] & 0x02) {
		xpos[1] = IT7260_X_RESOLUTION - (((buf[7] & 0x0F) << 8) | buf[6]);
		ypos[1] = IT7260_Y_RESOLUTION - (((buf[7] & 0xF0) << 4) | buf[8]);
		event[1] = buf[9] & 0x0F;
	}

	/* finger 2 */
	if (buf[0] & 0x04) {
		xpos[2] = IT7260_X_RESOLUTION - (((buf[11] & 0x0F) << 8) | buf[10]);
		ypos[2] = IT7260_Y_RESOLUTION - (((buf[11] & 0xF0) << 4) | buf[12]);
		event[2] = buf[13] & 0x0F;
	}

	cur_msec = jiffies_to_msecs(jiffies);
	if ((priv->data.event[0] != (!!event[0])) ||
	    (priv->data.event[1] != (!!event[1])) ||
	    (priv->data.event[2] != (!!event[2])) ||
	    (priv->data.report_msec > cur_msec) ||
	    (cur_msec - priv->data.report_msec) > interval) {
		for (i = 0; i < 3; i++) {
			if (xpos[i] < IT7260_X_RESOLUTION && ypos[i] < IT7260_Y_RESOLUTION) {
...
#ifdef DEBUG_IT7260_TS
				dev_info(&priv->client->dev, "finger %d > xpos = %d, ypos = %d, event = %d\n",
						i, xpos[i], ypos[i], event[i]);
#endif
			}
		}
		input_sync(priv->input);
		...
		for (i = 0; i < 3; i++) {
			priv->data.xpos[i] = xpos[i];
			priv->data.ypos[i] = ypos[i];
			priv->data.event[i] = !!event[i];
		}
		priv->data.report_msec = cur_msec;
	}
...
#endif
	input_set_abs_params(input, ABS_MT_POSITION_X, 0, SCREEN_X_RESOLUTION - 1, 0, 0);
	input_set_abs_params(input, ABS_MT_POSITION_Y, 0, SCREEN_Y_RESOLUTION - 1, 0, 0);
...
MODULE_DESCRIPTION("it7260 touchscreen driver (multi touch)");
